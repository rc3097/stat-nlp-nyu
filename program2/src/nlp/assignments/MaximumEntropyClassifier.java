...
public class MaximumEntropyClassifier<I, F, L> implements
	...
	public static class ObjectiveFunction<F, L> implements
		...
		private Pair<Double, double[]> calculate(double[] x) {
			...
			double[][] prob = new double[data.length][indexLinearizer.numLabels];

			for(int i=0 ; i < (data.length) ; i++)
			{
				prob[i]=getLogProbabilities(data[i], x, encoding, indexLinearizer);
				objective+=prob[i][data[i].getLabelIndex()];
			}			

			
			for (int i =0; i<data.length;i++) {
				for (int j = 0; j < data[i].featureIndexes.length; j++) {
					int clabel=data[i].getLabelIndex();
					derivatives[indexLinearizer.getLinearIndex(data[i].getFeatureIndex(j), clabel)]+=data[i].getFeatureCount(j);
					for (int j2 = 0; j2 < indexLinearizer.numLabels; j2++) {
						int f = indexLinearizer.getLinearIndex(data[i].getFeatureIndex(j), j2);
						derivatives[f]-=(Math.exp(prob[i][j2])*data[i].getFeatureCount(j));
					}
				}
			}
			...
			objective -= 1 / (2 * sigma * sigma) * wsquare;
	...
	private static <F, L> double[] getLogProbabilities(EncodedDatum datum,
		...
		for (int i = 0; i < indexLinearizer.numLabels; i++) {
			logProbabilities[i] = 0.0;
			for (int j = 0; j < datum.featureIndexes.length; j++) {
				logProbabilities[i] += weights[indexLinearizer.getLinearIndex(datum.getFeatureIndex(j), i)]
