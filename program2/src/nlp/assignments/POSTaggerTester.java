...
public class POSTaggerTester {
	...
	static class HMMDecoder<S> implements TrellisDecoder<S> {
		public List<S> getBestPath(Trellis<S> trellis) {
			...
			List<S> pathList = new ArrayList<S>();
			S currentStatepair = (S) trellis.getStartState();
			HashMap<S, Double> currentList = new HashMap<S, Double>();
			List<HashMap<S, S>> pathMaps = new ArrayList<HashMap<S, S>>();
			currentList.put(currentStatepair, 0.0);
			boolean isEndState = false;
			while (!isEndState) {
				HashMap<S, Double> valueHolder = new HashMap<S, Double>();
				HashMap<S, S> tempPath = new HashMap<S, S>();
				Iterator<Entry<S, Double>> iterator = currentList.entrySet()
						.iterator();
				while (iterator.hasNext()) {
					Map.Entry mapEntry = (Map.Entry) iterator.next();
					S preState = (S) mapEntry.getKey();
					double preValue = (double) mapEntry.getValue();

					Counter<S> transitions = trellis
							.getForwardTransitions(preState);

					Iterator<Entry<S, Double>> iterator_transitions = transitions
							.getEntrySet().iterator();
					while (iterator_transitions.hasNext()) {
						Map.Entry possiblestateMapEntry = (Map.Entry) iterator_transitions
								.next();
						S possibleState = (S) possiblestateMapEntry.getKey();
						double possibleValue = (double) possiblestateMapEntry
								.getValue();
						if (possibleState.equals(trellis.getEndState())) {
							isEndState = true;
						}
						if (valueHolder.containsKey(possibleState)) {
							double oldvalue = valueHolder.get(possibleState);

							if (oldvalue <= preValue + possibleValue) {
								valueHolder.put(possibleState, preValue
										+ possibleValue);
								tempPath.put(possibleState, preState);
							}
						} else {
							valueHolder.put(possibleState, preValue
									+ possibleValue);
				...
				currentList = valueHolder;
				pathMaps.add(tempPath);
			}
			S state = trellis.getEndState();
			pathList.add(state);
			while (!state.equals(trellis.getStartState())) {
				HashMap<S, S> tempMap = pathMaps.get(pathMaps.size() - 1);
				state = tempMap.get(state);
				pathList.add(state);
				pathMaps.remove(pathMaps.size() - 1);
			}
	...
	static class MostFrequentTagScorer implements LocalTrigramScorer {
		...
		Counter<String> tagsCounter = new Counter<String>();
		Counter<String> unknownWordTags = new Counter<String>();
		Set<String> seenTagTrigrams = new HashSet<String>();
		HashMap<String, Integer> TagTrigrams = new HashMap<String, Integer>();
		HashMap<String, Integer> TagBigrams = new HashMap<String, Integer>();
		...
		ProbabilisticClassifierFactory<String, String> factory = new MaximumEntropyClassifier.Factory<String, String, String>(
				1.0, 60, new ProperNameFeatureExtractor());
		...
		public Counter<String> getLogScoreCounter(
			...
			if (wordsToTags.keySet().contains(word)) {
				...
				tagCounter = classifier.getProbabilities(word);
			}
			...
			for (String tag : tagCounter.keySet()) {
				double logScore;
					String tag1 = localTrigramContext.getPreviousPreviousTag();
					String tag2 = localTrigramContext.getPreviousTag();
					double PWT;
					PWT = (double) tagCounter.getCount(tag)
							/ tagsCounter.getCount(tag);
					if (getTriProbabilities(tag1, tag2, tag) != 0) {
						logScore = Math
								.log(getTriProbabilities(tag1, tag2, tag))
								+ Math.log(PWT);
					} else {
						logScore = Math.log(getBiProbabilities(tag2, tag))
								+ Math.log(PWT);
					}
		...
		public void train(
			...
			for (LabeledLocalTrigramContext labeledLocalTrigramContext : labeledLocalTrigramContexts) {
				...
				if (!wordsToTags.keySet().contains(word)) {
					...
					// Maxent
					LabeledInstance<String, String> datum = new LabeledInstance<String, String>(
							tag, new String(word));
					trainingData.add(datum);
				}
				wordsToTags.incrementCount(word, tag, 1.0);

				tagsCounter.incrementCount(tag, 1.0);
				...
				// trigram
				String temp = makeTrigramString(
						labeledLocalTrigramContext.getPreviousPreviousTag(),
						labeledLocalTrigramContext.getPreviousTag(),
						labeledLocalTrigramContext.getCurrentTag());
				if (TagTrigrams.containsKey(temp)) {
					int value = TagTrigrams.get(temp);
					value++;
					TagTrigrams.put(temp, value);
				} else {
					TagTrigrams.put(temp, 1);
				}
				temp = makeBigramString(
						labeledLocalTrigramContext.getPreviousPreviousTag(),
						labeledLocalTrigramContext.getPreviousTag());
				if (TagBigrams.containsKey(temp)) {
					int value = TagBigrams.get(temp);
					value++;
					TagBigrams.put(temp, value);
				} else {
					TagBigrams.put(temp, 1);
				}
			}
		...
		public double getTriProbabilities(String tag1, String tag2, String tag3) {
			String temp = makeTrigramString(tag1, tag2, tag3);
			if (TagTrigrams.containsKey(temp)) {
				return (double) TagTrigrams.get(temp)
						/ TagBigrams.get(makeBigramString(tag1, tag2));
			}
			return 0;
		}

		public double getBiProbabilities(String previousTag, String currentTag) {
			String temp = makeBigramString(previousTag, currentTag);
			if (TagBigrams.containsKey(temp)) {
				return (double) TagBigrams.get(temp)
						/ tagsCounter.getCount(previousTag);
			}
			return 0;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		TrellisDecoder<State> trellisDecoder = new HMMDecoder<State>();
